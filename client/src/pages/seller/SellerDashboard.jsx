...
const SellerDashboard = () => {
    ...
    const { fetchSellerProducts, addProduct, deleteProduct } = useProducts();
    ...
    useEffect(() => {
        const loadSellerData = async () => {
            ...
            try {
                ...
                const prodData = await fetchSellerProducts(user.id);
    ...
    const handleDelete = (id) => {
        if (window.confirm('Are you sure you want to remove this product?')) {
            deleteProduct(id);
            setSellerProducts(prev => prev.filter(p => p.id !== id && p._id !== id));
        }
    };
    ...
    const filteredInventory = sellerProducts.filter(p =>
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
                ...
                {user && (
                    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="flex items-center gap-4 bg-white px-5 py-2.5 rounded-full shadow-sm border border-stone-200">
                        <div className="w-10 h-10 bg-linear-to-br from-terracotta-400 to-terracotta-600 rounded-full flex items-center justify-center text-white font-bold shadow-md">
                            {user.name.charAt(0)}
                        </div>
                                ...
                                <div className="bg-white p-8 rounded-2xl shadow-sm border border-stone-100 relative overflow-hidden">
                                    ...
                                    <div className="h-80 w-full relative z-10">
                                        <ResponsiveContainer width="100%" height="100%">
                                            <BarChart data={chartData.length > 0 ? chartData : [{ name: 'No Data', sales: 0 }]} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                        ...
                        {activeTab === 'products' && (
                            ...
                            >
                                <div className="px-8 py-6 border-b border-stone-100 flex flex-col sm:flex-row justify-between items-center gap-4 bg-stone-50/50">
                                    <h3 className="font-bold text-lg text-stone-800">Inventory ({sellerProducts.length})</h3>
